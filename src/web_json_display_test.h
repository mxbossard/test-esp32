...
void sendHttpGet(WiFiClient* client, const char* hostHeader, const char* endpoint) {
    Serial.println("GET " + String(endpoint) + " HTTP/1.0");
    Serial.println("Host: " + String(hostHeader)); 
    Serial.println("Connection: close");
    Serial.println();

    client->println("GET " + String(endpoint) + " HTTP/1.0");
...
void loop(void) {
    ...
    client.begin(webpage); //Specify the URL
    ...
    if (httpCode == 200) {
        ...
        ReadLoggingStream wifiClientWithLog(client.getStream(), Serial);

        //sendHttpGet(&client, httpHost, httpEndpoint);
        
        DeserializationError err = deserializeJson(doc, wifiClientWithLog);
